  ...
  const RegistrationScreen = () => {
    ...
    const [formData, setFormData] = useState({
      ...
      confirmPassword: "",
      idPhotoType: "",
      photoOfYourself: null, // Separate state for the first image
      photoOfYourId: null, // Separate state for the second image
    });
...
<input
  type="file"
  accept="image/*"
  onChange={(e) => handleFileChange(e, 'photoOfYourself')} // Update this line
  required
/>
      </label>
      {formData.photoOfYourself && (
        <div className="flex items-center gap-2 mt-2">
          <img
            src={URL.createObjectURL(formData.photoOfYourself)}
            alt="Uploaded Photo"
  ...
  type="file"
  accept="image/*"
  onChange={(e) => handleFileChange(e, 'photoOfYourId')} // Update this line
  required
/>
      </label>
      {formData.photoOfYourId && (
        <div className="flex items-center gap-2 mt-2">
          <img
            src={URL.createObjectURL(formData.photoOfYourId)}
            alt="Uploaded Photo"
            className="uploaded-photo w-48 h-56"
          />
        </div>
