...
const exportAttendanceToCsv = async (filePath, examId) => {
  // Find all exams and populate related user data
  const exams = await TakeExam.find({ exam: examId })
    .populate("exam", "name") // Adjust field names as per your `Exam` schema
    .populate("user", "email")
    .populate("userAnswers", "score")
    .exec();
  ...
  const records = exams.map((exam) => ({
    ...
    score: exam.userAnswers ? exam.userAnswers.score : "N/A",
  }));
