...
exports.evaluateUserAnswer = catchAsync(async (req, res, next) => {
  ...
  const exam = await Exam.findById(userAnswer.examId).populate("questions");
  ...
  for (let i = 0; i < userAnswer.questionAnswers.length; i++) {
    ...
    for (let j = 0; j < questions.length; j++) {
      if (
        questions[j]._id.equals(userAnswer.questionAnswers[i].questionId._id)
      ) {
    ...
    if (question) {
      ...
      const answerText = userAnswer.questionAnswers[i].answerText
        .toLowerCase()
        .trim();
